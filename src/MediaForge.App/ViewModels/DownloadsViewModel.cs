...
public partial class DownloadsViewModel : ObservableObject
{
    ...
    [ObservableProperty] private string _statusText = "הדבק קישור ל־YouTube כדי להתחיל";
    ...
    private async Task ResolveAsync(CancellationToken cancellationToken)
    {
        ...
        IsBusy = true; Items.Clear(); CollectionTitle = string.Empty; StatusText = "בודק את הקישור ומביא פרטי מדיה…";
        try
        {
            ...
            StatusText = result.IsPlaylist ? $"נמצאו {Items.Count} פריטים בפלייליסט" : "נמצא שיר אחד — אפשר לערוך לפני הוספה";
    ...
    [RelayCommand]
    private void SelectAll(bool? selected) { var value = selected ?? true; foreach (var item in Items) item.IsSelected = value; }

    [RelayCommand]
    private void Mp3ToAll()
    {
        foreach (var item in Items) { item.DesiredFormat = MediaFormat.Mp3; item.IsSelected = true; }
    ...
    {
        if (IsBusy) return;
        if (string.IsNullOrWhiteSpace(DestinationDirectory)) { StatusText = "בחר תיקיית יעד"; return; }
        var selected = Items.Where(x => x.IsSelected).Select(x => x.ToResolvedItem()).ToArray();
        if (selected.Length == 0) { StatusText = "בחר לפחות פריט אחד"; return; }
        IsBusy = true;
        try
        {
            ...
            StatusText = staged.Count == selected.Length ? $"{staged.Count} הורדות נוספו לשינויים ממתינים" : $"{staged.Count} הורדות נוספו; פריטים כפולים דולגו";
        }
        catch (OperationCanceledException) { StatusText = "הפעולה בוטלה"; }
        catch (Exception ex) { StatusText = ex.Message; }
        finally { IsBusy = false; }
...
public partial class ResolvedMediaItemViewModel : ObservableObject
{
    ...
    public ResolvedMediaItemViewModel(ResolvedMediaItem item, IReadOnlyList<MediaFormat> formats) { _item = item; _title = item.Metadata.Title; _desiredFormat = item.DesiredFormat; Formats = formats; }
    public ResolvedMediaItem ToResolvedItem() => _item with { Metadata = _item.Metadata with { Title = Title.Trim() }, DesiredFormat = DesiredFormat };
}
