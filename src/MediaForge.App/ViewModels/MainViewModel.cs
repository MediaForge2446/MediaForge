...
public partial class MainViewModel : ObservableObject
{
    ...
    public MainViewModel(LibraryService libraryService, LibraryScanService libraryScanService, IFolderPicker folderPicker,
        StagingService stagingService, ICommitEngine commitEngine, ExplorerViewModel explorer,
        DownloadsViewModel downloads, SettingsViewModel settings)
    {
    ...
    {
        IsBusy = true;
        StatusText = "סורק את המחשב ומסנכרן את הספרייה…";
        try
        {
    ...
    private async Task RefreshLibraryAsync(CancellationToken cancellationToken)
    {
        if (IsBusy) return;
        IsBusy = true;
        StatusText = "סורק מחדש את התיקיות במחשב…";
        try
        {
            foreach (var root in RootFolders) await RefreshRootAsync(root, cancellationToken).ConfigureAwait(true);
            await Explorer.ReloadAsync(cancellationToken).ConfigureAwait(true);
            StatusText = "הספרייה עודכנה לפי מצב הדיסק";
        }
    ...
    private async Task NavigateAsync(string section, CancellationToken cancellationToken)
    {
        ...
        PageTitle = section switch { "home" => "הספרייה שלי", "explorer" => "סייר הקבצים", "downloads" => "הורדת מדיה", "settings" => "הגדרות וכלים", _ => "הספרייה שלי" };
        switch (section)
        {
            case "home": CurrentPage = this; break;
            case "explorer":
                if (string.IsNullOrWhiteSpace(Explorer.CurrentPath)) await Explorer.InitializeAsync(RootFolders.FirstOrDefault()?.Path, cancellationToken).ConfigureAwait(true);
                CurrentPage = Explorer; break;
            case "downloads": CurrentPage = Downloads; break;
    ...
    private async Task AddRootFolderAsync(CancellationToken cancellationToken)
    {
        ...
        IsBusy = true;
        StatusText = "מוסיף וסורק תיקייה…";
        try
        {
            var library = await _libraryService.LoadAsync(cancellationToken).ConfigureAwait(true);
            if (library.RootFolders.Any(x => string.Equals(x.Path, path, StringComparison.OrdinalIgnoreCase))) { StatusText = "התיקייה כבר קיימת בספרייה"; return; }
            var root = await _libraryService.AddRootFolderAsync(library, path, cancellationToken: cancellationToken).ConfigureAwait(true);
            var vm = new RootFolderViewModel(root);
            RootFolders.Add(vm);
            await RefreshRootAsync(vm, cancellationToken).ConfigureAwait(true);
            if (string.IsNullOrWhiteSpace(Downloads.DestinationDirectory)) Downloads.SetDefaultDestination(root.Path);
            if (string.IsNullOrWhiteSpace(Explorer.CurrentPath)) await Explorer.SetInitialPathAsync(root.Path, cancellationToken).ConfigureAwait(true);
            OnPropertyChanged(nameof(HasLibrary));
            StatusText = "התיקייה נוספה ונבדקה מול הדיסק";
        }
    ...
    private async Task RemoveRootFolderAsync(RootFolderViewModel? folder, CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            OnPropertyChanged(nameof(HasLibrary));
            StatusText = "התיקייה הוסרה מהספרייה בלבד — הקבצים במחשב לא נמחקו";
        }
        catch (OperationCanceledException) { StatusText = "הפעולה בוטלה"; }
    ...
    private async Task SaveChangesAsync(CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            RefreshPendingCount();
            StatusText = result.Success ? "כל השינויים נשמרו ואומתו בדיסק" : $"השמירה הסתיימה עם {result.Items.Count(x => !x.Success)} שגיאות";
            await Explorer.ReloadAsync(cancellationToken).ConfigureAwait(true);
            foreach (var root in RootFolders) await RefreshRootAsync(root, cancellationToken).ConfigureAwait(true);
        }
...
public sealed partial class RootFolderViewModel : ObservableObject
{
    ...
    {
        Exists = scan.Exists; FolderCount = scan.FolderCount; FileCount = scan.FileCount; MediaCount = scan.MediaCount; TotalSizeText = scan.TotalSizeText;
        ScanStatus = scan.Exists ? "מסונכרן עם הדיסק" : "התיקייה לא נמצאה";
        ScanSummary = scan.Exists ? $"{scan.MediaCount} קבצי מדיה · {scan.FileCount} קבצים · {scan.FolderCount} תיקיות · {scan.TotalSizeText}" : "יש לעדכן את הנתיב או לחבר את הכונן";
    }
}
