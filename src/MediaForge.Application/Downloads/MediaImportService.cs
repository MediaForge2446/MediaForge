...
public sealed class MediaImportService
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        var usedVideoIds = new HashSet<string>(_staging.Operations.Select(x => x.Payload?.VideoId).Where(x => !string.IsNullOrWhiteSpace(x)).Select(x => x!), StringComparer.OrdinalIgnoreCase);
        ...
        foreach (var item in items)
        {
            ...
            var operation = new StagingOperation(
                ...
                new StagingPayload(SourceUrl: item.SourceUrl, DestinationPath: destinationPath, DesiredFormat: format, VideoId: item.VideoId));
    ...
    private static string MakeUnique(string baseName, string extension, ISet<string> usedNames)
    {
        ...
        var index = 2;
        while (!usedNames.Add(candidate)) candidate = $"{baseName} ({index++}){extension}";
        return candidate;
    }
